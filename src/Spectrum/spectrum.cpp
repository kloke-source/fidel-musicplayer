...
spectrum::spectrum() {
  for (size_t band = 0; band < spect_bands; band++) {
    this->spect_band_magnitudes.push_back(0);
    shaders.push_back(0);
...
void spectrum::init_connections() {
  this->bands_updated_connection =
      audio_playback::Instance()->signal_spect_bands_updated().connect(
          sigc::mem_fun(this, &spectrum::on_spect_bands_updated));
  this->playback_status_changed_connection =
      audio_playback::Instance()->signal_status_changed().connect(
          sigc::mem_fun(this, &spectrum::on_playback_status_changed));
}

void spectrum::set_spect_bands(int bands) { this->spect_bands = bands; }
...
void spectrum::start_visualization() {
  this->visualization_connection = Glib::signal_timeout().connect(
      sigc::mem_fun(*this, &spectrum::on_timeout), 1);
}

void spectrum::stop_visualization() { visualization_connection.disconnect(); }
...
void spectrum::clear_context(const Cairo::RefPtr<Cairo::Context> &cr) {
  ...
  if (auto_padding == true)
    spect_padding = (double)(.65 * frame_width) /
                    (spect_bands + 1); // .65 is ideal padding ratio

  util::set_source_rgb(cr, "#2d2d2d");
  cr->rectangle(0, 0, frame_width, frame_height);
  cr->fill();
  double spec_bar_width =
      (frame_width - ((spect_bands + 1) * spect_padding)) / spect_bands;
  util::set_source_rgb(cr, "#36D7B7");

  for (int band = 0; band < spect_bands; band++) {
    double bar_x_pos =
        spect_padding + (band * (spec_bar_width + spect_padding));
...
void spectrum::set_spect_band_magn(int band, double magn) {
  this->spect_band_magnitudes[band] = magn;
}
...
bool spectrum::on_draw(const Cairo::RefPtr<Cairo::Context> &cr) {
  std::cout << "On draw called" << std::endl;
  ...
  for (int band = 0; band < spect_band_magnitudes.size(); band++) {
    ...
    double height_required =
        frame_height - (((-1) * magnitude) / spectrum_vert_scale);

    double interp_x_pos = (shaders[band] * scale_factor * PI) /
                          speed; // interpolation variables (the x position on
                                 // the interpolation curve)

    double bar_height;
    if (scale_factor != 0 && audio_player::Instance()->is_playing() == true) //if (scale_factor != 0 && audio_playback::Instance()->is_playing() == true)
      bar_height =
          scale_factor *
              spectrum::sin_func((1 / scale_factor) * interp_x_pos - PI / 2) +
          scale_factor; // interpolation variables

    if (audio_player::Instance()->is_playing() == false)//if (audio_playback::Instance()->is_playing() == false)
      bar_height = previously_painted[band];

    bar_heights[band] = bar_height;

    if (previously_painted[band] < height_required &&
        shaders[band] <= subdivisions) {
      shaders[band]++;
    ...
    cr->rectangle(bar_x_pos, (frame_height / 2 - bar_heights[band] / 2),
                  spec_bar_width, bar_heights[band]);
