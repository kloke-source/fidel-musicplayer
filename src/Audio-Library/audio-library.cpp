...
enum { INFO_ALBUM_ID, INFO_ALBUM_NAME, INFO_ALBUM_ART };
...
int AudioLibrary::generic_db_callback(void *data, int total_col_num,
                                      char **value, char **fields) {
...
/**
 * \brief Essentially initializes the sqlite database so that it can be written
 * to \details This function initializes the database as an in-memory database
 * so that database related operations take place much quicker
 */
void AudioLibrary::init_db() {
  if (db_initialized == false) {
    ...
    /* Open database */
    ret_code = sqlite3_open_v2(
        "", &library_db, SQLITE_OPEN_READWRITE,
        NULL); /**< By leaving the first parameter sqlite creates a temporary
                  database which has comparable speeds to a full fledged
                  in-memory database */
    if (ret_code) {
    ...
    ret_code = sqlite3_exec(library_db, util::to_char(sql_stmt),
                            generic_db_callback, 0, &error_msg);
    if (ret_code != SQLITE_OK) {
      if (util::has_text(error_msg, "already exists") ==
          false) // omits the already exists error msg
...
void AudioLibrary::load_db() {
  ...
  if (db_loaded == false) {
    ...
    sqlite3 *source_file;       /* Database connection opened on
                                   default_db_location.c_str() */
    ...
    /* Open the database file identified by default_db_location.c_str(). Exit
     *early if this fails * for any reason. */
    ...
    /* Close the database connection opened on database file
     *default_db_location.c_str() * and return the result of this function. */
...
void AudioLibrary::load_album_info() {
  if (db_loaded == false) {
    ...
    sqlite3_prepare_v2(library_db, "SELECT * FROM album_information;", -1,
                       &statement, 0);
    ...
    while (sqlite3_column_text(statement, INFO_ALBUM_ID)) {
      ...
      album_info.album_name =
          std::string((char *)sqlite3_column_text(statement, INFO_ALBUM_NAME));
      full_loaded_album_names.insert(album_info.album_name);

      guint8 *raw_image_buffer =
          (guint8 *)sqlite3_column_blob(statement, INFO_ALBUM_ART);
      gsize raw_image_size = sqlite3_column_bytes(statement, INFO_ALBUM_ART);

      album_info.album_art =
          std::make_tuple(raw_image_buffer, raw_image_size, true);
...
std::tuple<guint8 *, gsize, bool>
AudioLibrary::get_album_art(std::string album_name) {
...
bool AudioLibrary::check_file_format(std::string input_file) {
  return valid_file_formats.check(
...
void AudioLibrary::db_ins_blob(std::string blob_ins_stmt, guint8 *buffer,
                               gsize buffer_size) {
  AudioLibrary::initialize();
  sqlite3_stmt *statement;
  int ret_code = sqlite3_prepare_v2(library_db, blob_ins_stmt.c_str(), -1,
                                    &statement, NULL);

  if (ret_code != SQLITE_OK)
    cerr << "prepare failed: " << sqlite3_errmsg(library_db) << endl;
  else {
    ret_code = sqlite3_bind_blob(statement, 1, (char *)buffer, buffer_size,
                                 SQLITE_STATIC);
    if (ret_code != SQLITE_OK) {
...
void AudioLibrary::write_lib_data(std::string file_location) {
  ...
  std::string ins_stmt =
      util::gen_ins_stmt("library", library_fields, library_field_values);
...
void AudioLibrary::write_lib_info() {
  ...
  std::string ins_stmt = util::gen_ins_stmt(
      "library_information", library_info_fields, library_info_values);
  AudioLibrary::db_ins_row(ins_stmt);
}
...
void AudioLibrary::write_album_summ() {
  ...
  for (size_t album_iter = 0; album_iter < full_album_summary.size();
       album_iter++) {
    std::string album_name = full_album_summary[album_iter].album_name;
    std::vector<std::string> songs_in_album =
        full_album_summary[album_iter].songs_in_album.inorder();

    for (size_t songs_iter = 0; songs_iter < songs_in_album.size();
         songs_iter++) {
      ...
      int file_loc_pos =
          full_album_summary[album_iter].songs_in_album.get_search_id();
      std::string file_location =
          full_album_summary[album_iter].file_locations[file_loc_pos];
      ...
      std::string ins_stmt = util::gen_ins_stmt(
          "album_summary", album_summ_fields, album_summ_values);
      AudioLibrary::db_ins_row(ins_stmt);
    }
  }
}
...
void AudioLibrary::write_album_info(std::string file_location) {
  ...
  if (album_found == false) {
    ...
    if (std::get<2>(album_info.album_art) == true) {
      ...
      blob_ins_stmt
          << "INSERT INTO album_information (album_name, album_art) VALUES('"
          << album_name << "', ?);";
      AudioLibrary::db_ins_blob(blob_ins_stmt.str(),
                                std::get<0>(album_info.album_art),
                                std::get<1>(album_info.album_art));
    } else {
      album_info_values.push_back("No Album Art");
      std::string ins_stmt = util::gen_ins_stmt(
          "album_information", album_info_fields, album_info_values);
      AudioLibrary::db_ins_row(ins_stmt);
    }
  }
}
  ...
  if (full_artist_summary.size() == 0) {
    ...
    for (size_t iter = 0; iter < full_artist_summary.size(); iter++) {
      if (full_artist_summary[iter].artist_name == artist_name) {
        artist_exists = true;
        if (full_artist_summary[iter].songs_by_artist.check(song_name) ==
            false) {
...
void AudioLibrary::write_artist_summ() {
  ...
  for (size_t artist_iter = 0; artist_iter < full_artist_summary.size();
       artist_iter++) {
    std::string artist_name = full_artist_summary[artist_iter].artist_name;
    std::vector<std::string> songs_by_artist =
        full_artist_summary[artist_iter].songs_by_artist.inorder();

    for (size_t songs_iter = 0; songs_iter < songs_by_artist.size();
         songs_iter++) {
      ...
      int file_loc_pos =
          full_artist_summary[artist_iter].songs_by_artist.get_search_id();
      std::string file_location =
          full_artist_summary[artist_iter].file_locations[file_loc_pos];
      ...
      std::string ins_stmt = util::gen_ins_stmt(
          "artist_summary", artist_summ_fields, artist_summ_values);
      AudioLibrary::db_ins_row(ins_stmt);
    }
  }
}
...
void AudioLibrary::load_album_index() {
  sqlite3_stmt *stmt;

  sqlite3_prepare_v2(library_db, "SELECT * from album_information", -1, &stmt,
                     0);

  int increment = 0;

  enum { ID, ALBUM, INDEXED_ALBUM_ART };

  while (increment != SQLITE_DONE) {
    increment = sqlite3_step(stmt);

    if (increment == SQLITE_ROW) {
      std::string album_name =
          util::to_string(sqlite3_column_text(stmt, ALBUM));
      guint8 *raw_img = (guint8 *)sqlite3_column_blob(stmt, INDEXED_ALBUM_ART);
      gsize raw_img_size = (gsize)sqlite3_column_bytes(stmt, INDEXED_ALBUM_ART);

      std::stringstream art_exists_stream;
      art_exists_stream << raw_img;

      if (art_exists_stream.str() != "No Album Art") {
        Glib::RefPtr<Gdk::PixbufLoader> loader = Gdk::PixbufLoader::create();
        loader->write(raw_img, raw_img_size);
        loader->close();

        Glib::RefPtr<Gdk::Pixbuf> pixbuf = loader->get_pixbuf();
        Gtk::Image *album_art = new Gtk::Image(pixbuf);
        audioinfo::add_album_art_to_index(
            album_art, album_name); // Adds album art to index to make later
                                    // retrieval faster
      }
    }
  }
...
void AudioLibrary::load_album_summary() {
  AudioLibrary::load_db();
  char *err_msg = 0;
  int ret_code = 0;

  const char *sql_stmt = "SELECT * from album_summary";
  const char *data = "";

  auto album_retr_cb = [](void *data, int total_col_num, char **value,
                          char **fields) {
    enum { ID, ALBUM_NAME, SONGS_IN_ALBUM, FILE_LOC };

    std::string album_name = value[ALBUM_NAME];
    std::string song_in_album = value[SONGS_IN_ALBUM];
    std::string file_loc = value[FILE_LOC];

    int end_pos = full_album_summary.size();

    if (end_pos > 0)
      end_pos = (full_album_summary.size()) - 1;

    std::cout << "Full album sum end pos -> " << end_pos << std::endl;

    if (!full_album_summary.empty()) {
      if (strcmp(full_album_summary[end_pos].album_name.c_str(),
                 album_name.c_str()) == 0) {
        full_album_summary[end_pos].songs_in_album.insert(song_in_album);
        full_album_summary[end_pos].file_locations.push_back(file_loc);
        full_album_summary[end_pos].total_songs_in_album++;
      } else {
        AlbumSummary album_summ;
        album_summ.album_name = album_name;
        album_summ.songs_in_album.insert(song_in_album);
        album_summ.file_locations.push_back(file_loc);
        album_summ.total_songs_in_album++;
        full_album_summary.push_back(album_summ);
      }
    } else {
      AlbumSummary album_summ;
  ...
  ret_code =
      sqlite3_exec(library_db, sql_stmt, album_retr_cb, (void *)data, &err_msg);
  ...
  /*
  for (size_t iter = 0 ; iter < full_album_summary.size(); iter++) {
    std::cout << "Album: " << full_album_summary[iter].album_name << std::endl;
    std::vector<std::string> songs =
  full_album_summary[iter].songs_in_album.inorder(); for (size_t song_iter = 0;
  song_iter < songs.size(); song_iter++) { std::cout << "   -> " <<
  songs[song_iter] << std::endl;
    }
  }
  */
}

void AudioLibrary::load_artist_summary() {}

std::vector<AlbumSummary> AudioLibrary::get_album_summary() {
  return full_album_summary;
}
...
void AudioLibrary::populate_playlist() {
  ...
  auto album_index_job = std::async(
      std::launch::async, [&]() { AudioLibrary::load_album_index(); });
  auto album_retr_job = std::async(
      std::launch::async, [&]() { AudioLibrary::load_album_summary(); });

  ret_code = sqlite3_exec(library_db, sql_stmt, populate_playlist_cb,
                          (void *)data, &err_msg);
  if (ret_code != SQLITE_OK) {
    fprintf(stderr, "SQL error: %s\n", err_msg);
    sqlite3_free(err_msg);
...
int AudioLibrary::populate_playlist_cb(void *data, int total_col_num,
                                       char **value, char **fields) {
...
void AudioLibrary::scan() {
  ...
  AudioLibrary::initialize();
  DirIndexer dir_indexer;

  // AudioLibrary::scan_dir(default_music_folder.c_str());
  std::function<void(std::string, struct dirent *)> test_cb =
      [](std::string file_location, struct dirent *) {
        // std::cout << "File location to add " << std::endl;
  ...
  dir_indexer.get_all_contents_in(default_music_folder, true, false, test_cb);
  ...
  std::cout << "Flushed to db" << std::endl;
  db_indexed = true;
  indexed_once = true;
  std::chrono::high_resolution_clock::time_point end_point =
      std::chrono::high_resolution_clock::now();
  auto scan_time = std::chrono::duration_cast<std::chrono::microseconds>(
...
for (size_t subdir_iter = 0; subdir_iter < subdir_locations.size();
     subdir_iter++) {
